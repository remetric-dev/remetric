...
func TestRenderReport_ReferenceLineWhenDocURLSet(t *testing.T) {
	rep := &findings.Report{
		Version:   "v0.1.0",
	...
	want := "**Reference:** [https://remetric.dev/findings/hot-label](https://remetric.dev/findings/hot-label)"
	if !strings.Contains(buf.String(), want) {
		t.Errorf("output missing %q, got:\n%s", want, buf.String())
	}
}

func TestRenderReport_NoReferenceLineWhenDocURLEmpty(t *testing.T) {
	rep := &findings.Report{
		Version:   "v0.1.0",
		ScannedAt: time.Date(2026, 5, 22, 0, 0, 0, 0, time.UTC),
		Findings: []findings.Finding{
